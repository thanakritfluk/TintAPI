...
color_prediction_route = Blueprint('color_prediction_route', __name__)

# This method executes before any API request
@color_prediction_route.before_request
def before_request():
    print('Start Color Prediction API request')

@color_prediction_route.route('/api/add/lipstick')
def insert_lipstick():
    lipstick = Lipstick('FlukLip', '#EBA38B', 500)
    insert = lipstick.insert()
    return("Success insert id:"+str(insert), 200)

@color_prediction_route.route('/api/lipstick/brand/list')
def get_lipstick_brand():
    brand = Lipstick.distinct_brand()
    return(JSONEncoder().encode(brand), 200)

@color_prediction_route.route('/api/lipstick/list/from/brand')
...
@color_prediction_route.route('/api/color/prediction', methods=['POST'])
@cross_origin()
def prediction():
...
@color_prediction_route.after_request
